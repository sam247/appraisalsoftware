...
Do not apply production migrations automatically. Rehearse against staging first. The new respondent routing requires the workflow RPCs even for annual personal links, so **apply both pending 360 migrations before deploying this frontend**. Follow with `20260922190000_enable_360_release.sql` so fresh environments open the operator release switch. `ENABLE_360_FEEDBACK` has been removed from the application; the product UI treats 360 as available once the database switch is on.
...
**Released.** Application gating via `ENABLE_360_FEEDBACK` has been removed and `private.feedback_360_release.enabled` is on. Continue validating complete annual/360 desktop and mobile paths, keyboard navigation, anonymous submission/closure concurrency, revoked/expired-token behaviour, hosted Supabase/PostgREST privileges and staged email delivery/cron configuration without emailing production customers.
